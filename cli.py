...
def push(subset, repos, config_file, branch_name, remote_name, repo_path, sandbox_path):
    ...
    for r in repo_list:
        try:
            push_changes(r, branch_name, remote_name)
        except Exception as e:
            click.echo(f"Failed to push changes in {r}: {e}")
...
def pr(subset, repos, config_file, branch_name, pr_title, pr_body, repo_path, sandbox_path):
    ...
    for r in repo_list:
        try:
            open_pull_request(r, branch_name, pr_title, pr_body)
        except Exception as e:
            click.echo(f"Failed to open PR for {r}: {e}")
